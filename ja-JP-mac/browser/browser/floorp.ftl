...
windows-chrome-useragent-mode = 
    .label = Windows の Chrome に偽装
macOS-chrome-useragent-mode = 
    .label = macOS の Chrome に偽装
linux-chrome-useragent-mode = 
    .label = Linux の Chrome に偽装
mobile-chrome-useragent-mode = 
    .label = iOS の Chrome に偽装
...
custom-URL-option = Web パネルの URL を設定
...
bsb-userAgent-label = 
    .label = モバイル用 Web サイトを表示
bsb-width = Web パネルの幅 ( 0 ならグローバル値が使われます)
bsb-page = 開くページ
bsb-add = ブラウザマネージャーサイドバーに Web パネルを追加
bsb-setting = Web パネルの設定
bsb-add-title = 
    .title = { bsb-add }
bsb-setting-title = 
    .title = { bsb-setting }
