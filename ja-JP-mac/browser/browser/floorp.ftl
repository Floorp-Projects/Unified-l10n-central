...
enable-addons-in-sidebar2-with-experimental = 
    .label = ブラウザマネージャーサイドバーで拡張機能のコンテンツスクリプトを有効にする (実験的)
enable-addons-in-sidebar2 = 
    .label = ブラウザマネージャーサイドバーで拡張機能のコンテンツスクリプトを有効にする
sidebar2-enable-addons-desc = 有効にすると、広告ブロックやその他の機能がサイドバーで動作するようになります。また、そのウェブページで使用可能な API の数も増えます。
...
block-fingerprint = フィンガープリントは、ブラウザーとオペレーティングシステム固有の機能に依存する追跡メカニズムです。このセクションには、デフォルトのブロック機能を超えてさらに強力に保護するための設定が含まれています。
