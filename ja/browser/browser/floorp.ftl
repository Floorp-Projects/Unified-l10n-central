...
multirow-tabs-value = 複数行タブが有効な場合の行数
enable-tab-sleep = 
    .label = タブのスリープを有効にする
tab-sleep-timeout-minutes-value = 非アクティブになった後、タブはスリープ状態になります (分)
tab-sleep-settings-button = 設定...
tab-sleep-settings-dialog-title = 
    .title = スリープ状態タブ設定
tab-sleep-settings-dialog-excludehosts-label = ホストを除外
tab-sleep-settings-dialog-excludehosts-label-2 = 1行に1つのホストを入力します。
...
tabbar-favicon-color = 
    .label = タブバーに現在のウェブサイトのfavicon色を使用して色を付ける
...
vertical-tabbar = 
    .label = 垂直タブバー (実験的)
native-tabbar-tip = サイドバーのアドオンと内蔵サイドバーは、Firefoxのサイドバーを活用するVertical Tabsと同時に使用することはできません。
native-vertical-tab-show-right = 
    .label = ウィンドウの右側に垂直タブバーを表示
hover-vertical-tab = 
    .label = 垂直タブバーを閉じる
TST = ツリースタイルタブ
about-TST = ツリー型タブは、ツリー構造でタブを表示することができる一般的なアドオンです。 Floorp 10 has a built this add-on. Please install the add-restore Floorp 10's built Tree Style Tab.
treestyletab-Settings = 
    .label = ツリースタイルタブを閉じる
sidebar-reverse-position-toolbar = 反対側にサイドバーを表示
bookmarks-bar-settings = ブックマークツールバー (一度に1つのオプションのみ使用)
bookmarks-focus-mode = 
    .label = ナビゲーションバーにカーソルを合わせない限りブックマークツールバーを非表示にする
bookmarks-bottom-mode = 
    .label = { -brand-short-name } の下部にブックマークツールバーを表示する
nav-bar-settings = ツールバー
show-nav-bar-bottom = 
    .label = { -brand-short-name } の下部にツールバーを表示します（実験的）
material-effect = 
    .label = みんなのためにマイカにブラウザのデザインを変更することを許可する
disable-extension-check-compatibility-option = 
    .label = アドオンとの互換性を確認しない
other-preference = その他の設定
enable-userscript = 
    .label = レガシーコンポーネントを有効にする
about-legacy-components = この機能を有効にすると、予期しないバグや致命的なエラーが発生する可能性があります。
Search-positon-top = 
    .label = ページ上部に検索バーを表示する
allow-auto-restart = 
    .label = 再起動が必要な設定が変更された場合、自動的に再起動します
browser-rest-mode = 
    .label = 残りモードのショートカットを有効にする (F9)
disable-fullscreen-notification = 
    .label = 全画面表示時に通知を表示しない
floorp-updater = { -brand-short-name } アップデート
enable-floorp-updater = 
    .label = 起動時に { -brand-short-name } のアップデートを確認する
floorp-update-latest = 
    .label = 自動更新のチェック中に { -brand-short-name } が最新の状態になった場合に通知する

## Browser Theme

system-color-settings = テーマによっては、明るいモードと暗いモードの両方があります。テーマを使用したいモードを選択します。
preferences-theme-appearance-header = テーマモード
system-theme-dark = 
    .label = ダーク
system-theme-light = 
    .label = ライト
...
finish-always-notify = 
    .label = ダウンロードが終了したときにのみ通知します
always-notify = 
    .label = ダウンロードが終了したときに通知する
do-not-notify = 
    .label = ダウンロード通知を無効にする
floorp-translater = 翻訳者の設定
click-to-option = 
    .label = 設定を開く...
    .accesskey = O
...
profiles-button-label = プロファイルの管理
...
default-useragent-mode = 
    .label = Firefox User Agent (デフォルト) を使用する
windows-chrome-useragent-mode = 
    .label = WindowsでChromeを偽装する
macOS-chrome-useragent-mode = 
    .label = macOS で Chrome を偽装する
linux-chrome-useragent-mode = 
    .label = Linux で Chrome を偽装する
mobile-chrome-useragent-mode = 
    .label = iOSでChromeを偽装する
...
download-mgr-UI = 
    .label = SimpleUI ダウンロードマネージャを有効にする
downloading-red-color = 
    .label = ダウンロード時に赤いダウンロードアイコンを使用する
sidebar-preferences = Sidebar
bsb-preferences = ブラウザーマネージャのサイドバー設定
view-sidebar2-right = 
    .label = ブラウザマネージャの右側にサイドバーを表示する
enable-sidebar2 = 
    .label = ブラウザマネージャのサイドバーを有効にする
visible-bms = 
    .label = ブラウザマネージャのサイドバーを表示
hide-bms-to-unload-panel = 
    .label = パネルを非表示にした時にパネルをアンロードする
...
bsb-header = ブラウザーマネージャのサイドバー
bsb-context = 次のコンテナタブを使用します
bsb-userAgent-label = 
    .label = この Web パネルで Mobile User Agent を使用する
bsb-width = 幅 (0 に設定されている場合、グローバル値が使用されます)
bsb-page = ページを開く
bsb-add = ブラウザマネージャサイドバーにウェブパネルを追加
...
sidebar2-pref-setting = 
    .label = 設定
sidebar2-global-width = グローバル Web パネルの幅
use-icon-provider-option = アイコンプロバイダーを使用する
...
balance-memory = 
    .label = 残高メモリ使用量とパフォーマンス
max-memory = 
    .label = 最高速度とパフォーマンス (高メモリ使用量)
...
newtab-background-folder-open = 
    .label = フォルダを開く
newtab-background-folder-choose = 画像フォルダを選択...
newtab-background-extensions = これらのファイル拡張子を持つ画像を使用してください (",")
disable-blur-on-newtab = 
    .label = { -brand-short-name } ホームのぼかし効果を無効にする

## Lepton Preferences

about-lepton = Leptonで { -brand-short-name } をカスタマイズ
lepton-preference-button = 
    .label = ルプトン設定...
    .accesskey = L
lepton-header = レプトンの設定
lepton-preference = レプトンの設定
photon-mode = 
    .label = Photonデザインを使用する
lepton-mode = 
    .label = レプトンのデザインを使用
protonfix-mode = 
    .label = プロトンのデザインを微調整する
autohide-preference = ブラウザーの要素を自動的に非表示にする
...
floorp-lepton-enable-overlap-sidebar = 
    .label = ウェブサイトのコンテンツにサイドバーを表示する
floorp-home-mode-choice-default = 
    .label = { -brand-short-name } ホーム (デフォルト)
floorp-home-prefs-content-header = { -brand-short-name } ホームコンテンツ
floorp-home-prefs-content-description = { -brand-short-name } ホームページで表示したいコンテンツを選択します。

## Notes

floorp-notes = { -brand-short-name } メモ
restore-from-backup = バックアップからメモを復元する
enable-notes-sync = 
    .label = { -brand-short-name } ノートの同期を有効にする
about-notes-backup-tips = Floorp Notes は Firefox Sync を使用してノートを他のデバイスと同期します。 ノートを紛失した場合は、バックアップから復元できます。 { -brand-short-name } を起動するとバックアップが作成されます。
notes-sync-description = これにより、同期中にノートを上書きすることでコンテンツが失われる問題が解決します。
...
restore-from-this-backup = ノートをバックアップ中の状態に戻しますか？
restore-button = 復元
...
userjs-prompt = このuser.jsを適用しますか？
apply-userjs-attention = 新しいuser.jsを適用すると、現在のuser.jsファイルが上書きされます。
apply-userjs-attention2 = 続行する前に、現在のuser.jsファイルをバックアップしてください。
userjs-button = user.js 設定...
userjs-select-option = パフォーマンスとプライバシーを向上させるために現在 { -brand-short-name } で使用されているuser.jsを管理します。
apply-userjs-button = 適用

## userjs Options

default-userjs-label = Floorp デフォルト
about-default-userjs = テレメトリーを無効にしました。さまざまなカスタマイズが有効なバランスの取れた { -brand-short-name } です。
Securefox-label = Yokoffing Securefox
about-Securefox = HTTPS-Default。サイト分離による完全なクッキー保護。強化された状態とネットワーク分割。その他様々な機能強化。
default-label = Yokoffing Default
about-default = すべての必須項目。破損のどれもありません。これはuser.jsです。
Fastfox-label = Yokoffing Fastfox
about-Fastfox = Firefoxのブラウジング速度を向上させます。Chromeにそのお金のための実行を与えます！
...
about-Smoothfox = 好みのブラウザーでエッジのようなスムーズなスクロールを取得するか、スタイルを選択してください。
...
floorp-CSK-description = { -brand-short-name }のキーボードショートカットをカスタマイズします。Floorpは80以上のカスタマイズ可能なキーボードショートカットを提供します。 { -brand-short-name } を再起動してこれらの設定を適用します。 重複したキーボードショートカットは機能しません。
disable-fx-actions = 
    .label = Mozilla Firefoxのキーボードショートカットを無効にする
customize-Action = 
    .label = ショートカットを追加
remove-Action = 
    .label = ショートカットを削除
...
CSK-remove-shortcutkey = カスタムキーボードショートカットを削除しますか？
CSK-remove-shortcutkey-description = このキーボードショートカットを削除してもよろしいですか？
CSK-restore-default = デフォルトに戻しますか？
CSK-restore-default-description = Floorpのデフォルトキーボードショートカットを復元しますか？現在のショートカットは失われます。
...
CSK-keyborad-shortcut-is-changed = (適用されません)
floorp-custom-actions-tab-action = タブアクション
floorp-custom-actions-page-action = ページアクション
floorp-custom-actions-visible-action = 表示するアクション
...
floorp-custom-actions-open-page-action = ページアクションを開く
...
floorp-custom-actions-bms-action = ブラウザーマネージャのサイドバーアクション
...
# Translate
TWS = ウェブページを翻訳
about-TWS = GoogleやYandexを使用して、リアルタイムでページを翻訳します。また、選択したテキストやページ全体を翻訳することもできます。
...
block-tracker = このセクションには、広告とトラッカーをブロックするように設計されたエクステンションのセットが含まれています。
view-at-AMO = addons.mozilla.orgでこのアドオンを表示
uBlock-Origin = uBlock Origin
about-uboori = uBlock Originは、広告、広範なトラッカー、およびその他の危険なサイトをブロックします。
Facebook-Container = Facebook コンテナ
about-Facebook-Container = Facebookがあなたを追跡するのを防ぎます。Facebook Container extensionは、あなたがコントロールし、Facebookからあなたのウェブ活動を分離するのに役立ちます。

## Fingerprinting

fingerprint-header = 指紋認証とIPアドレスの漏洩を無防備にする
block-fingerprint = フィンガープリントは、ブラウザとオペレーティングシステムの固有の機能に依存する追跡メカニズムです。 このセクションには、デフォルトのブロックを超えてこの保護をさらに強化するための設定が含まれています。
enable-firefox-fingerprint-protections = 指紋認証に対する強力な保護を有効にする
about-firefox-fingerprint-protection = Firefox による保護を有効にするには強制ライトモードが含まれています。一部の API などを無効にします。一部のサイトが壊れている可能性があります。
fingerprint-Protection = 
    .label = フィンガープリント対策保護
html5-canvas-prompt-settings = 
    .label = HTML5 画像データのアクセス確認プロンプトを自動的に却下する
canvas-prompt = ウェブサイトが手動で許可されない限り、canvas-reading prompt を使用しないようにします。
disable-webgl = 
    .label = WebGL を無効にする
about-webgl = WebGL はグラフィックをレンダリングするために使用される Javascript API で、GPU を識別するために使用できます。
Canvas-Blocker = キャンバスブロッカー
about-CB = このアドオンは fingerprinting 技術で使用される偽装データです。
WebRTC-connection = WebRTCはリアルタイム通話を提供する標準です。この設定を無効にするとDiscordなどを使用できなくなります。
WebRTC = 
    .label = WebRTC接続を有効にする
################################################################### browser ###############################################################

rest-mode = 休憩中...
rest-mode-description = フロアは現在停止されています。続行するにはENTERまたはOKを押してください。
Sidebar2 = 
    .label = ブラウザーマネージャのサイドバー
    .tooltiptext = サイドバーの表示を変更
...
sidebar-keepWidth-button = 
    .tooltiptext = このパネルの現在の幅を使い続ける
sidebar2-keep-width-for-global = 
    .label = この幅をカスタム幅なしですべてのパネルに適用する
bsb-context-add = 
    .label = ウェブパネルにページを追加...
...
css-menubar = 
    ...
    .accesskey = C
rebuild-css = 
    .label = ブラウザのCSSファイルを再構築
    .accesskey = R
make-browsercss-file = 
    .label = ブラウザ CSS ファイルを作成
    .accesskey = M
open-css-folder = 
    .label = CSSフォルダを開く
    .accesskey = O
edit-userChromeCss-editor = 
    .label = userChrome.cssファイルを編集
edit-userContentCss-editor = 
    .label = userContent.cssファイルを編集
not-found-editor-path = CSSファイルエディタが見つかりませんでした
...
gf-floorp-open-browser-manager-sidebar-description = ブラウザーマネージャのサイドバーを開きます。ブラウザーマネージャのサイドバーがロードされている場合。
