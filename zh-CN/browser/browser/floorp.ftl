...
multirow-tabs-newtab = 
    .label = 将"打开一个新标签"按钮放置在最低标签页行末端
multirow-tabs-value = 启用多行标签时的行数
enable-tab-sleep = 
    .label = 启用睡眠标签
tab-sleep-timeout-minutes-value = 标签将在禁用后睡眠(分钟)
tab-sleep-settings-button = 设置...
tab-sleep-settings-dialog-title = 
    .title = 睡眠标签设置
tab-sleep-settings-dialog-excludehosts-label = 排除主机
tab-sleep-settings-dialog-excludehosts-label-2 = 每行输入一个主机。
...
windows-chrome-useragent-mode = 
    .label = Windows 上的Spoof Chrome
macOS-chrome-useragent-mode = 
    .label = macOS 上的Spoof Chrome
linux-chrome-useragent-mode = 
    .label = Linux上的 Chrome Spoof
mobile-chrome-useragent-mode = 
    .label = iOS 上的Spoof Chrome
...
bsb-context = 使用以下容器选项卡
bsb-userAgent-label = 
    .label = 在此网页面板中使用移动用户代理
...
newtab-background-extensions = 使用带有这些文件扩展名的图像(用",")
...
floorp-lepton-disable-userChrome-icon = 
    .label = 禁用Lepton的上下文菜单和面板菜单图标
floorp-lepton-disable-userChrome-menu-icon = 
    .label = 启用Lepton的上下文菜单图标
...
floorp-notes = { -brand-short-name } 说明
restore-from-backup = 从备份恢复备注
enable-notes-sync = 
