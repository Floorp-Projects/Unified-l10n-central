...
multirow-tabs-newtab = 
    .label = 将 “打开新标签页” 按钮放在最下面一行标签页的末尾
...
enable-floorp-workspace = 
    .label = 启用工作空间
workspace-warning = 工作空间无法与标签页分组扩展一同使用。如果您想要使用标签页分组扩展，请禁用工作空间并重新启动 { -brand-short-name }。
...
native-vertical-tab-show-right = 
    .label = 在窗口右侧显示垂直标签栏
TST = Tree Style Tab
about-TST = Tree Style Tab 是一个受欢迎的附加组件，允许您以树状结构显示标签页。 Floorp 10 内置了这个附加组件。请安装此附加组件以恢复 Floorp 10 内置的垂直标签栏功能。
