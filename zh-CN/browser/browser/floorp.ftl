...
about-notes-backup-tips = { -brand-short-name } 笔记使用 Firefox Sync 与其他设备同步您的笔记。如果您丢失了笔记，可以从备份中恢复。当您启动 { -brand-short-name } 时，就会创建一个备份。
...
floorp-custom-actions-workspace-action = 工作区操作
...
gf-floorp-open-tree-style-tab-description = 打开侧边栏的 Tree Style Tab 面板
gf-floorp-open-bookmarks-sidebar-name = [{ -brand-short-name }] 打开侧边栏的书签面板
gf-floorp-open-bookmarks-sidebar-description = 打开侧边栏的书签面板
gf-floorp-open-history-sidebar-name = [{ -brand-short-name }] 打开侧边栏的历史记录面板
gf-floorp-open-history-sidebar-description = 打开侧边栏的历史记录面板
gf-floorp-open-synctabs-sidebar-name = [{ -brand-short-name }] 打开侧边栏的同步标签页面板
...
gf-floorp-open-extension-sidebar-name = [{ -brand-short-name }] 打开选中的侧边栏插件
...
memo-third-tip = { -brand-short-name } 笔记将会保存在您的 { -brand-short-name } 设置中，并通过 Firefox Sync 在不同设备之间同步。Firefox Sync 会使用您的 Firefox 账户密码对同步的内容进行加密，因此除了您自己，没有其他人能够访问其内容。
...
welcome-enable-basic-features = 启用基本功能和设置，实现简单的体验。
...
shortcut-key-description = 点击 “开始监听” 并输入新的键盘快捷键以执行此操作。可以使用多个键，但如果已经分配给其他操作，键盘快捷键将不起作用。
